...
def get_next_open_port():
  ...
  This is a docker specific funtion that collects all ports used by docker and returns
  a port that is higher by a random amount from a currently known highest used port.

  This function relates directly with chosen deployment cenario: if port contentention is not 
  expected then this function is not needed.
  '''
...
def create_new_instance():
  '''
  Starts an actual MySql docker container and creates a database and a user for the database.

  This function expects certain environment variables to be present in order to function.

  Return dictionary includes values that the API expects. 
  '''
...
def create_instance_user():
  '''
  Function creates a single database user and sets correct access restrictions.

  This is used in other commands from API and therefore was sparated out from instance creation.

  This function expects certain environmental values to be present in order to function. Thus, for now,
  we forgo function arguments and directly use values from calling environment (set by Salt).
  '''
  if not verify_environment():
    sys.exit(1)
...
json_message = create_new_instance()
json_message["command"] = "createInstance"
